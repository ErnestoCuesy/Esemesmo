...
class _PaymentListViewState extends State<PaymentListView> {
  ...
  final snackBarExcluded = SnackBar(
    content: Text('Payment excluded'),
  );
  final snackBarIncluded = SnackBar(
    content: Text('Payment included'),
  );
          ...
          itemBuilder: (BuildContext context, int index) {
            return Dismissible(
              ...
              background: Container(
                color: Colors.white,
                child: globalData.transactions[index].excludeFlag
                    ? Icon(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        ...
                        Container(
                          ...
                          height: 20.0,
                          child: Text(
                            globalData.transactions[index].manualEntry
                                ? "M/E"
                    ...
                    ListTile(
                      ...
                      subtitle: Padding(
                          padding: const EdgeInsets.only(left: 0.0),
                          child: Text(Utilities.formatDateTime(
                              globalData.transactions[index].date))),
                      trailing: Padding(
                        padding:
                            const EdgeInsets.only(bottom: 16.0, right: 8.0),
                        child: Text(
                            f.format(globalData.transactions[index].amount)),
                      ),
                    ),
                  ],
                ),
  ...
  Text categoryName(int index) {
    ...
    Text catName = Text("${globalData.categories[catIndex].name}",
        style: globalData.transactions[index].category == 0
            ? textTheme.subtitle.copyWith(color: Colors.pink)
  ...
  Widget categoryFilterMenuButton() {
    return PopupMenuButton<String>(
        ...
        itemBuilder: (BuildContext context) {
          int uncategorized = globalData.transactionsMaster
              .where((tx) => tx.category == 0 && !tx.excludeFlag)
              .length;
          int excluded = globalData.transactionsMaster
              .where((tx) => tx.excludeFlag)
              .length;
          int categorized = globalData.transactionsMaster
              .where((tx) => tx.category != 0 && !tx.excludeFlag)
              .length;
          int all = globalData.transactionsMaster
              .where((tx) => !tx.excludeFlag)
              .length;
          return [
            SHOW_UNCATEGORIZED + '($uncategorized)',
            ...
            return PopupMenuItem<String>(
                child: Text(filterItem), value: filterItem);
          }).toList();
        });
  }
  ...
  void _sendViaEmail() {
    Navigator.of(context).push(EmailCapture()).then((_) {
      print(
          "Sending ${globalData.transactions.length} transactions to ${globalData.preferences.recipientEmailAddress}");
  ...
  void _showUpdatePaymentsCategoryDialog(Category category, int index) {
    if (globalData.transactions[index].category != category.id) {
      ...
      vendorCount =
          globalData.transactions.where((tx) => tx.vendor == testVendor).length;
      if (vendorCount > 1) {
        showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text(
                    "Would you like to change $vendorCount ${globalData.transactions[index].vendor} payments to this category?"),
                actions: <Widget>[
                  TextButton(
                    ...
                    onPressed: () {
                      ...
                      dBProvider
                          .updateTransaction(globalData.transactions[index]);
                      ...
                      setState(() {
                        globalData.transactions.forEach((transaction) {
                          if (transaction.vendor ==
                              globalData.transactions[index].vendor) {
                            transaction.category = category.id;
                          }
                        });
                      });
                      dBProvider.updateAllVendorTransactions(
                          globalData.transactions[index].vendor, category.id);
                      if (_appBarCategory.contains("uncategorized")) {
                        globalData.transactions.clear();
                        PaymentServices.copyTransactions(
                            COPY_UNCATEGORIZED, 0, 0);
                      }
                      Navigator.of(context).pop();
                    },
                  )
